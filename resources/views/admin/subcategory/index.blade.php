...
@section('content')
    <div class="container-fluid">
        ...
        <div class="d-sm-flex align-items-center justify-content-between mb-4">
            ...
            <a href="{{ route('admin.subcategory.create') }}" class="btn btn-danger btn-icon-split btn-sm">
                                    ...
                                    @if ($subcategories->isNotEmpty())
                                        @foreach ($subcategories as $key => $item)
                                            <tr>
                                                ...
                                                <td class="text-center">
                                                    <a href="{{ route('admin.subcategory.edit', $item->id) }}" class="btn btn-warning btn-circle btn-sm" title="Edit"><i class="fas fa-edit"></i></a>
                                                    <button type="button" class="btn btn-danger btn-circle btn-sm" title="Delete" onclick="modalDelete({{ $item->id }}, '{{ route('admin.subcategory.delete', 'ID') }}')"><i class="fas fa-trash"></i></button>
                                                </td>
                                            </tr>
                                        @endforeach
                                    @else
